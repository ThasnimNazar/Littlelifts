...
const Weekendsitting: React.FC<WeekendsittingProps> = ({ selectedOptionid }) => {
    ...
    const handleTimeChange = (
        ...
        // Validation and overlapping check
...
const handleSaveDates = async () => {
    ...
    const formattedAvailableDates = availableDates.map(date => ({
        ...
        timeslots: date.timeslots.map(slot => ({
            startTime: slot.startTime ? formatTimeWithoutSeconds(slot.startTime) : null,
            endTime: slot.endTime ? formatTimeWithoutSeconds(slot.endTime) : null,
        })),
    }));
    ...
    return (
        <>
            ...
            <Box className="mt-10 p-4 flex justify-center">
                <Weekendcalendar handleDateSelect={handleDateSelect} />
            </Box>
