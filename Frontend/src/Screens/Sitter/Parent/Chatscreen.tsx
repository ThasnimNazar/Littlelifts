...
const Chatscreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChat = async () => {
      if (selectedSitter) {
        try {
          const response = await parentApi.post('/createchat', {
            parentId: parentId,
          ...
          const messagesResponse = await parentApi.get(`/get-messages/${chatData._id}`);
  ...
  const handleSendMessage = async (message: Message, image: File | null, video: File | null, audio: File | null) => {
    if (chatId && parentId && (message.content.trim() || image || video || audio)) {
      try {
        ...
        const response = await parentApi.post('/send-message', formData, {
          headers: {
  ...
  const handleMarkSeen = () => {
    if (chatId && parentId) {
      parentApi.post('/mark-seen', { chatId, userId: parentId });
      if (socket) {
