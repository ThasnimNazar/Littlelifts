...
import Sitterforgetpassword from './Sitterforgetpassword';
...
const Sitterchat: React.FC = () => {
  ...
  useEffect(() => {
    const fetchChat = async () => {
      if (selectedParent) {
        try {
          const response = await axios.post('/api/sitter/createchat', {
            sitterId: sitterId,
          ...
          const messagesResponse = await axios.get(`/api/sitter/get-messages/${chatData._id}`);
  ...
  const handleSendMessage = async (message: Message, image: File | null, video: File | null, audio: File | null) => {
    if (chatId && sitterId && (message.content.trim() || image || video || audio)) {
      try {
        ...
        const response = await axios.post('/api/sitter/send-message', formData, {
          headers: {
  ...
  const handleMarkSeen = () => {
    if (chatId && sitterId) {
      axios.post('/api/sitter/mark-seen', { chatId, userId: sitterId });
      if (socket) {
